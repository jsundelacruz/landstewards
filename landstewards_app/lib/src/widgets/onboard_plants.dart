import 'package:flutter/material.dart';
import 'package:landstewards_app/src/experience_controller.dart';
import 'package:landstewards_app/src/plants_controller.dart';
import 'package:landstewards_app/src/widgets/option_selector.dart';

class OnboardPlants extends StatefulWidget {
  final PageController pageController;
  final PlantsFilterController controller;
  const OnboardPlants({required this.pageController, required this.controller, super.key});

  @override
  State<OnboardPlants> createState() => _OnboardPlantsState();
}

class _OnboardPlantsState extends State<OnboardPlants> {
  @override
  Widget build(BuildContext context) {
    return Container(
      padding: const EdgeInsets.all(32.0),
      color: const Color(0xFFF2F0DB),
      // constraints: BoxConstraints(maxWidth: 800, maxHeight: MediaQuery.of(context).size.height - 70),
      child: Column(
        mainAxisAlignment: MainAxisAlignment.center,
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          const Spacer(),
          const Text(
            'What types of plants do you want to grow here?',
            style: TextStyle(color: Color(0xFF45423F), fontSize: 28.0, fontWeight: FontWeight.w700),
          ),
          const SizedBox(height: 16.0),
          const Text(
            'We’ll focus on native plants that will grow well in your area.',
            style: TextStyle(color: Color(0xFF45423F), fontSize: 16.0),
          ),
          const SizedBox(height: 16.0),
          Wrap(
            spacing: 5.0,
            runSpacing: 5.0,
            children: PlantsFilterTypes.values.map((PlantsFilterTypes option) {
              return FilterChip(
                  backgroundColor: const Color(0xFFF2F0DB),
                  showCheckmark: false,
                  padding: const EdgeInsets.symmetric(vertical: 12.0, horizontal: 8.0),
                  label: Row(
                    mainAxisSize: MainAxisSize.min,
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      Icon(
                        widget.controller.selectedFilters.contains(option) ? Icons.check : Icons.add,
                        color: const Color(0xFF45423F),
                      ),
                      const SizedBox(width: 8),
                      Text(
                        widget.controller.getEnumDescription(option),
                        style: const TextStyle(
                            color: Color(0xFF45423F), fontSize: 16.0, fontWeight: FontWeight.w500, height: 1.5),
                        textAlign: TextAlign.left,
                        softWrap: true,
                      ),
                    ],
                  ),
                  selectedColor: const Color(0x995D7B1E),
                  selected: widget.controller.selectedFilters.contains(option),
                  onSelected: (bool selected) {
                    setState(() {
                      if (selected) {
                        widget.controller.selectedFilters.add(option);
                      } else {
                        widget.controller.selectedFilters.remove(option);
                      }
                    });
                  });
            }).toList(),
          ),
          const Spacer(),
          SizedBox(
            width: double.infinity,
            child: ElevatedButton(
                style: ElevatedButton.styleFrom(
                  elevation: 0,
                  backgroundColor: const Color(0xFF45423F),
                  padding: const EdgeInsets.symmetric(horizontal: 16.0, vertical: 20.0),
                ),
                onPressed: widget.controller.selectedFilters.isNotEmpty
                    ? () {
                        Navigator.pushNamed(context, '/location');
                      }
                    : null,
                child: Text('show my plant recommendations'.toUpperCase(),
                    style: const TextStyle(color: Color(0xFFF2F0DB), fontSize: 14.0, fontWeight: FontWeight.w700))),
          ),
        ],
      ),
    );
  }
}
